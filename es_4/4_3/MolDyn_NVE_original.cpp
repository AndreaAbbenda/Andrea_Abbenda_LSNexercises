...
// errors
  ...
  for(int istep=1; istep <= nstep; ++istep){
	...
	if(istep%100 == 0){
	   errEpot.open("output_err_solid_epot.dat",ios::app);
  	   errEkin.open("output_err_solid_ekin.dat",ios::app);
  	   errTemp.open("output_err_solid_temp.dat",ios::app);
  	   errEtot.open("output_err_solid_etot.dat",ios::app);
    	   ...
    	   sum2etot = 0, sum2epot = 0, sum2ekin = 0, sum2temp = 0;
    	   errEpot.close();
	   errEkin.close();
	   errTemp.close();
	   errEtot.close();
...
//  double ep, ek, pr, et, vir;
  ...
  cout << "Simulating solid phase " << endl;
...
//Kinetic energy
  for (int i=0; i<npart; ++i) t += 0.5 * (vx[i]*vx[i] + vy[i]*vy[i] + vz[i]*vz[i]);
    ...
    stima_pot = 120*v/(double)npart; //Potential energy per particle      //modifico le unità di misura affinché i risultati non siano
    stima_kin = 120*t/(double)npart; //Kinetic energy per particle        //in unità ridotte ma in S.I. nel caso di Argon
    stima_temp = 120*(2.0 / 3.0) * t/(double)npart; //Temperature         //(m = 39.948 amu, sigma = 0.34 nm, epsilon/k = 120 K)
    stima_etot = 120*(t+v)/(double)npart; //Total energy per particle
