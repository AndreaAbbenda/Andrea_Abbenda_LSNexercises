...
Chromosome::Chromosome(int* array, int n){
	N=n;
	C=new int[N];
	for (int i=0;i<N;i++) C[i]=array[i];
...
void Population::Accumulate(int shape, int rank){
	
	ofstream L, AveL;
	if (shape == 0){
		L.open("L1_circle_all.dat",ios::app);
		AveL.open("AveL1_circle_all.dat",ios::app);
	} else {
		L.open("L1_square_all.dat",ios::app);
		AveL.open("AveL1_square_all.dat",ios::app);
	}
	
	L << rank <<"         "<<Fitness(0,1)<<"          "<<Fitness(0,2)<<endl;
