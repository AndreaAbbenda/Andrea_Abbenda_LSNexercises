...
//	return k_elastic*val;
	return k_elastic*val*val*val*4.0 - 5.0*val;
}
...
double external_potential_second(double val)
{
	double k_elastic=1;
//	return k_elastic;
	return k_elastic*val*val*12.0 - 5.0;
}
...
double variationalWaveFunction(double x)
{
	return 1.0;
/*	double _mu = 0.8017;                    
	double _sigma = 0.639731;
	double a = exp(-pow(x-_mu,2)/(2*_sigma*_sigma)) + exp(-pow(x+_mu,2)/(2*_sigma*_sigma));
	double norm = sqrt(2 * _sigma * sqrt(M_PI)* (1+exp(-_mu*_mu/_sigma/_sigma)));
	return a / norm;
*/
}
...
/*	double _mu = 0.8017;                    
	double _sigma = 0.639731;
	double a = exp(-pow(x-_mu,2)/(2*_sigma*_sigma)) + exp(-pow(x+_mu,2)/(2*_sigma*_sigma));
	double b = exp(-pow(x+_mu,2)/(2*_sigma*_sigma)) - exp(-pow(x-_mu,2)/(2*_sigma*_sigma));
	double sigma4 = _sigma*_sigma*_sigma*_sigma;
	double norm = sqrt(2 * _sigma * sqrt(M_PI)* (1+exp(-_mu*_mu/_sigma/_sigma)));
	return (a * (x*x + _mu*_mu + _sigma*_sigma)/sigma4 + b * 2*x*_mu/sigma4) / norm;
*/
}
...
{
	ofstream out("potential_cost_IT4.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("kinetic_cost_IT4.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("probability_cost_IT4.dat");
